...
| POST | /trend | 좌표 → 상권코드 조인 → 서울시 상권변화지표(생존율×폐업율 등급) 조회. 기술적 구현만 완료된 상태이며, 어떤 산출값/설명으로 노출할지는 아직 미정. 폴리곤 GeoJSON+API키 미설정 시 `available:false`로 정직하게 응답 |
...
## 다음 단계 (선택)
- `/trend`(상권변화지표 조회)를 실제로 어떻게 쓸지 — 젠트리피케이션 관련 근거로 쓸지, 다른 지표와
  합칠지, 화면에 어떤 라벨로 노출할지 — 아직 논의 중. 최종 산출값(화면에 보여줄 지표) 목록 자체도
  아직 확정 전이라, 여기 결론 나기 전엔 설계 원칙으로 문서화하지 않는다.
- `data/sanggwon.geojson`(서울시 영역-상권 GeoJSON) 확보 필요 — 없으면 `/trend`는 상권코드 조인 자체가 안 됨.
